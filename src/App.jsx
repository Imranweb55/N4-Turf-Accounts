...
function StatCard({ label, value, color, sub, progress }) {
  const colors = {
    green: "#1a6b38", blue: "#1565c0",
    amber: "#b45309", purple: "#5b21b6",
    red: "#c0392b", gray: "#444",
  };
  return (
    <div style={{
      ...
      <div style={{ fontSize: 10, color: "#999", marginBottom: 4, fontWeight: 600, textTransform: "uppercase", letterSpacing: 0.3 }}>
        {label}
      </div>
      <div style={{ fontSize: 20, fontWeight: 800, color: colors[color] || "#444" }}>
        {value}
      </div>
      ...
      {progress !== undefined && (
        <div style={{ height: 5, background: "#eee", borderRadius: 3, marginTop: 7 }}>
          <div style={{ height: "100%", borderRadius: 3, background: "#1a472a", width: `${progress}%` }} />
        </div>
      )}
    </div>
  );
...
export default function App() {
  ...
  const [settings, setSettings] = useLocalStorage("turf_settings_v2", DEFAULT_SETTINGS);
  ...
  const totalPaid = (parseFloat(addGpay) || 0) + (parseFloat(addCash) || 0) + (parseFloat(addAdvance) || 0);
  ...
  const endMins = addSlotEnd ? timeToMinutes(addSlotEnd) + (parseInt(settings.slotDur) || 60) : 0;
  ...
  useEffect(() => {
    const now = new Date();
    const mk = now.toLocaleDateString("en-IN", { month: "short", year: "numeric" });
    setHistMonth(mk);
  }, []);
  ...
  }, [addSlot, addSlotEnd, addDate, settings.slotDur, settings.wdPrice, settings.wePrice]);
  ...
  const fetchDayBookings = useCallback(async (date) => {
    ...
    try {
      ...
      setDayBookings(bookings[date] || []);
      showToast(bookings[date]?.length ? "⚠️ Offline — showing cached data" : "❌ No data. Check internet + Script URL.");
    }
  ...
  function resetAddForm() {
    setAddName(""); setAddPhone(""); setAddNotes("");
    setAddDiscount(0); setAddDiscReason("");
    setAddGpay(0); setAddCash(0); setAddAdvance(0); setAddAdvMode("");
    // Reset amount to auto-calculated value
    setAddAmount(autoCalcAmount);
  }

  // ── STEP 13: HISTORY LOAD ────────────────────────────────────
  const loadHistory = useCallback(async (monthKey) => {
    setHistLoading(true);
    setAllHistBookings([]);
    try {
      const data = await sheetFetch(settings.scriptUrl, {
      ...
      Object.keys(bookings).forEach((date) => {
        ...
        if (mk === monthKey) {
          (bookings[date] || []).forEach((b) => fallback.push({ ...b, date }));
        }
      });
  ...
  const dashStats = (() => {
    let total = 0, gpay = 0, cash = 0, adv = 0, pending = 0;
  ...
  const filteredHistory = allHistBookings.filter((b) => {
    if (histFilter === "pending") {
      const fin = parseFloat(b.finalAmount) || parseFloat(b.amount) || 0;
      const paid = (parseFloat(b.payGpay) || 0) + (parseFloat(b.payCash) || 0) + (parseFloat(b.payAdvance) || 0);
      return fin > paid;
    }
  ...
  async function syncAll() {
    ...
    setSyncing(false);
    showToast(count > 0 ? `✅ Synced ${count} booking(s)!` : "✅ All synced already");
  }

  async function testConn() {
    if (!settings.scriptUrl) { showToast("❌ Set Script URL in Settings first"); return; }
    showToast("🔌 Testing...");
    try {
      const d = await sheetFetch(settings.scriptUrl, { action: "ping" });
      showToast(d.status === "ok" ? "✅ Connected! " + (d.message || "") : "⚠️ " + d.message);
  ...
  return (
    <div style={{ maxWidth: 480, margin: "0 auto", minHeight: "100vh", display: "flex", flexDirection: "column", background: "#f0f4f0", fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif", fontSize: 15, color: "#1a1a1a" }}>
      ...
      {showInstallBanner && (
        <div style={{
          ...
          <div style={{ flex: 1 }}>
            <div style={{ fontWeight: 700, fontSize: 13 }}>Install Turf Manager</div>
            <div style={{ fontSize: 11, opacity: 0.8 }}>Add to home screen for quick access</div>
          </div>
          <button onClick={handleInstall} style={{
            background: "#fff", color: "#1a472a", border: "none",
      ...
      {page === "dashboard" && (
        <div style={{ display: "flex", flexDirection: "column", flex: 1, paddingBottom: 80, overflowY: "auto" }}>
          {/* Header */}
          <div style={{ background: "#1a472a", color: "#fff", padding: "14px 16px", display: "flex", alignItems: "center", gap: 10 }}>
            <div style={{ fontSize: 24 }}>⚽</div>
            <div>
              <h1 style={{ fontSize: 18, fontWeight: 700, margin: 0 }}>{settings.turfName}</h1>
              <p style={{ fontSize: 11, opacity: 0.65, marginTop: 1 }}>
                {new Date(dashDate + "T00:00:00").toLocaleDateString("en-IN", { weekday: "long", day: "numeric", month: "long" })}
              </p>
            </div>
            <div style={{ marginLeft: "auto" }}>
              <button
                onClick={syncAll}
                disabled={syncing}
                style={{ background: "rgba(255,255,255,0.15)", border: "1px solid rgba(255,255,255,0.3)", color: "white", padding: "6px 11px", borderRadius: 8, fontSize: 12, cursor: "pointer", opacity: syncing ? 0.6 : 1 }}
              >
          ...
          {/* Date bar */}
          <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "10px 14px", background: "#fff", borderBottom: "1px solid #eee" }}>
            {/* Controlled input: value={dashDate} + onChange = React controls this input */}
            <input
              type="date"
              value={dashDate}
              onChange={(e) => setDashDate(e.target.value)}
              style={{ flex: 1, border: "1.5px solid #ddd", borderRadius: 10, padding: "8px 10px", fontSize: 14, color: "#1a1a1a", background: "#f8f8f8", outline: "none" }}
            />
            <span style={{ background: isWeekend(dashDate) ? "#fff3e0" : "#e8f5e9", color: isWeekend(dashDate) ? "#e65100" : "#1a6b38", fontSize: 11, fontWeight: 600, padding: "4px 10px", borderRadius: 6, whiteSpace: "nowrap" }}>
              {isWeekend(dashDate) ? "Weekend" : "Weekday"} · ₹{isWeekend(dashDate) ? settings.wePrice : settings.wdPrice}
            </span>
            <button onClick={() => setDashDate(todayStr())} style={{ background: "#1a472a", color: "#fff", border: "none", borderRadius: 10, padding: "8px 13px", fontSize: 13, fontWeight: 600, cursor: "pointer" }}>
              Today
            </button>
          </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, padding: "0 14px 12px" }}>
            <StatCard label="Total Collected" value={fmtMoney(dashStats.total)} color="green" sub={`${dayBookings.length} booking(s) · ${dashStats.bookedSlots} slot(s)`} />
            <StatCard label="Occupancy" value={`${dashStats.occ}%`} color="gray" progress={dashStats.occ} />
            <StatCard label="📱 GPay Today" value={fmtMoney(dashStats.gpay)} color="blue" />
            <StatCard label="💵 Cash Today" value={fmtMoney(dashStats.cash)} color="amber" />
            <StatCard label="🗓 Advance Today" value={fmtMoney(dashStats.adv)} color="purple" />
            <StatCard label="⏳ Pending Due" value={fmtMoney(dashStats.pending)} color="red" />
          </div>

          {/* Slots list */}
          <div style={{ padding: "0 14px 6px", display: "flex", justifyContent: "space-between" }}>
            <h2 style={{ fontSize: 14, fontWeight: 700 }}>Slots</h2>
            <span style={{ fontSize: 11, color: "#aaa" }}>{slots.length} total slots</span>
          </div>

          {dashLoading ? (
            <div style={{ textAlign: "center", padding: 28, color: "#aaa", fontSize: 13 }}>⏳ Loading from Sheets…</div>
          ) : (
            <SlotsList
              slots={slots}
      ...
      {page === "add" && (
        <div style={{ display: "flex", flexDirection: "column", flex: 1, paddingBottom: 80, overflowY: "auto" }}>
          <div style={{ background: "#1a472a", color: "#fff", padding: "14px 16px", display: "flex", alignItems: "center", gap: 10 }}>
            <div style={{ fontSize: 24 }}>➕</div>
            <div>
              <h1 style={{ fontSize: 18, fontWeight: 700, margin: 0 }}>New Booking</h1>
              <p style={{ fontSize: 11, opacity: 0.65, marginTop: 1 }}>
                {isWeekend(addDate) ? "Weekend" : "Weekday"} · ₹{getRateForDate(addDate, settings)}/hr
              </p>
            </div>
          </div>
          <div style={{ padding: "12px 14px", flex: 1 }}>
            ...
            <FieldGroup label="Date">
              <input type="date" className="field-input" value={addDate} onChange={(e) => setAddDate(e.target.value)}
                style={inputStyle} />
            </FieldGroup>

            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 13 }}>
              <FieldGroup label="Start Time">
                <select value={addSlot} onChange={(e) => setAddSlot(e.target.value)} style={inputStyle}>
              ...
              <FieldGroup label="End Time">
                <select value={addSlotEnd} onChange={(e) => setAddSlotEnd(e.target.value)} style={inputStyle}>
                  {slots.map((s) => <option key={s.label} value={s.label}>{s.label}</option>)}
                </select>
              </FieldGroup>
            </div>
            ...
            {/* Duration info bar */}
            <div style={{ background: "#e8f5e9", borderRadius: 10, padding: "10px 13px", marginBottom: 13, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <div style={{ fontSize: 12, color: "#1a6b38" }}>
                ⏱ {durationHrs.toFixed(1)} hrs × {fmtMoney(rate)}/hr
              </div>
              <div style={{ fontSize: 16, fontWeight: 800, color: "#1a472a" }}>{fmtMoney(autoCalcAmount)}</div>
            </div>

            <FieldGroup label="Customer Name">
              <input type="text" value={addName} onChange={(e) => setAddName(e.target.value)} placeholder="Team / Customer name" style={inputStyle} />
            </FieldGroup>

            <FieldGroup label="Phone">
              <input type="tel" value={addPhone} onChange={(e) => setAddPhone(e.target.value)} placeholder="9876543210" maxLength={10} style={inputStyle} />
            </FieldGroup>

            <FieldGroup label="Sport">
              <select value={addSport} onChange={(e) => setAddSport(e.target.value)} style={inputStyle}>
                {SPORTS.map((s) => <option key={s}>{s}</option>)}
              </select>
            </FieldGroup>

            <FieldGroup label="Total Amount ₹ (auto-calculated, can edit)">
              <input type="number" value={addAmount} onChange={(e) => setAddAmount(parseFloat(e.target.value) || 0)} style={inputStyle} />
            </FieldGroup>

            {/* Discount */}
            <div style={{ background: "#fce4ec", borderRadius: 10, padding: "10px 12px", marginBottom: 13 }}>
              <div style={{ fontSize: 11, fontWeight: 700, color: "#c2185b", marginBottom: 6, textTransform: "uppercase", letterSpacing: 0.4 }}>Discount (₹)</div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
                <input type="number" value={addDiscount || ""} onChange={(e) => setAddDiscount(parseFloat(e.target.value) || 0)} placeholder="0"
                  style={{ ...inputStyle, background: "#fff" }} />
                <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                  <span style={{ fontSize: 11, color: "#888" }}>Final:</span>
                  <span style={{ fontSize: 17, fontWeight: 800, color: "#1a472a" }}>{fmtMoney(finalAmount)}</span>
                </div>
              </div>
              <input type="text" value={addDiscReason} onChange={(e) => setAddDiscReason(e.target.value)} placeholder="Reason for discount..."
                style={{ ...inputStyle, fontSize: 12, padding: "7px 10px", marginTop: 4, background: "#fff" }} />
            </div>

            {/* Payment fields */}
            <div style={{ background: "#f8f8f8", borderRadius: 10, padding: 12, marginBottom: 13 }}>
              <div style={{ fontSize: 11, fontWeight: 700, color: "#888", marginBottom: 8, textTransform: "uppercase", letterSpacing: 0.4 }}>Payment Received</div>
              {[
                { label: "📱 GPay", val: addGpay, set: setAddGpay },
                { label: "💵 Cash", val: addCash, set: setAddCash },
              ].map(({ label, val, set }) => (
                <div key={label} style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
                  <div style={{ fontSize: 12, color: "#777", fontWeight: 600, minWidth: 58 }}>{label}</div>
                  <input type="number" value={val || ""} onChange={(e) => set(parseFloat(e.target.value) || 0)} placeholder="0"
                    style={{ flex: 1, border: "1.5px solid #e0e0e0", borderRadius: 8, padding: "8px 10px", fontSize: 14, background: "#fff", outline: "none" }} />
                </div>
              ))}
              {/* Advance with mode selector */}
              <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
                <div style={{ fontSize: 12, color: "#777", fontWeight: 600, minWidth: 58 }}>🗓 Advance</div>
                <input type="number" value={addAdvance || ""} onChange={(e) => setAddAdvance(parseFloat(e.target.value) || 0)} placeholder="0"
                  style={{ flex: 1, border: "1.5px solid #e0e0e0", borderRadius: 8, padding: "8px 10px", fontSize: 14, background: "#fff", outline: "none" }} />
                {/* Toggle buttons for advance mode */}
                {["gpay", "cash"].map((m) => (
                  <button key={m} onClick={() => setAddAdvMode(addAdvMode === m ? "" : m)}
                    style={{ padding: "5px 10px", borderRadius: 8, fontSize: 12, fontWeight: 700, cursor: "pointer",
                      ...
                      background: addAdvMode === m ? (m === "gpay" ? "#e3f2fd" : "#fffde7") : "#fafafa",
                      color: addAdvMode === m ? (m === "gpay" ? "#1565c0" : "#b45309") : "#999" }}>
                    {m === "gpay" ? "📱" : "💵"}
                  </button>
                ))}
              </div>
              {/* Payment summary row */}
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", paddingTop: 8, borderTop: "1px solid #eee", marginTop: 4 }}>
                <div style={{ fontSize: 11, color: "#888" }}>Total paid: <span style={{ fontWeight: 700, color: "#1a472a" }}>{fmtMoney(totalPaid)}</span></div>
                <div style={{ fontSize: 11, color: "#c0392b" }}>Pending: <span style={{ fontWeight: 700 }}>{fmtMoney(pendingAmount)}</span></div>
              </div>
            </div>

            <FieldGroup label="Notes">
              <input type="text" value={addNotes} onChange={(e) => setAddNotes(e.target.value)} placeholder="Any notes..." style={inputStyle} />
            </FieldGroup>

            <button onClick={saveBooking} style={{ width: "100%", padding: 14, background: "#1a472a", color: "#fff", border: "none", borderRadius: 12, fontSize: 16, fontWeight: 700, cursor: "pointer", marginTop: 4 }}>
              Save Booking
            </button>
          </div>
        </div>
      ...
      {page === "history" && (
        <div style={{ display: "flex", flexDirection: "column", flex: 1, paddingBottom: 80, overflowY: "auto" }}>
          <div style={{ background: "#1a472a", color: "#fff", padding: "14px 16px", display: "flex", alignItems: "center", gap: 10 }}>
            <div style={{ fontSize: 24 }}>📋</div>
            <div><h1 style={{ fontSize: 18, fontWeight: 700, margin: 0 }}>History</h1><p style={{ fontSize: 11, opacity: 0.65, marginTop: 1 }}>{histMonth}</p></div>
          </div>

          <div style={{ padding: "10px 14px 4px" }}>
            <select value={histMonth} onChange={(e) => setHistMonth(e.target.value)} style={{ ...inputStyle, fontSize: 13 }}>
          ...
          <div style={{ display: "flex", gap: 7, padding: "10px 14px", overflowX: "auto" }}>
            {["all", "GPay", "Cash", "Advance", "Mix", "pending"].map((f) => (
              <button key={f} onClick={() => setHistFilter(f)}
                style={{ border: `1.5px solid ${histFilter === f ? "#1a472a" : "#e0e0e0"}`, borderRadius: 20, padding: "5px 13px", fontSize: 12, whiteSpace: "nowrap", cursor: "pointer",
                  background: histFilter === f ? "#1a472a" : "#fff", color: histFilter === f ? "#fff" : "#666", fontWeight: 600 }}>
          ...
          <div style={{ padding: "0 14px 10px" }}>
            <button
              onClick={() => {
                ...
                autoTable(pdf, {
                  startY: 34,
                  head: [["Date", "Slot", "Customer", "Sport", "Final Amt", "GPay", "Cash", "Advance", "Pending"]],
                  body: weekData.map((b) => {
                    const fin  = parseFloat(b.finalAmount) || parseFloat(b.amount) || 0;
                    ...
                    return [
                      ...
                      gp  > 0 ? `₹${Math.round(gp).toLocaleString("en-IN")}` : "-",
                      ca  > 0 ? `₹${Math.round(ca).toLocaleString("en-IN")}` : "-",
                      ad  > 0 ? `₹${Math.round(ad).toLocaleString("en-IN")}` : "-",
                      due > 0 ? `₹${Math.round(due).toLocaleString("en-IN")}` : "✓",
                    ];
                  }),
                  // Table styling
                  headStyles: { fillColor: [26, 71, 42], textColor: 255, fontStyle: "bold", fontSize: 8 },
                ...
                const totalGpay    = weekData.reduce((s, b) => s + (parseFloat(b.payGpay)    || 0), 0);
                const totalCash    = weekData.reduce((s, b) => s + (parseFloat(b.payCash)    || 0), 0);
                const totalAdv     = weekData.reduce((s, b) => s + (parseFloat(b.payAdvance) || 0), 0);
                const totalFin     = weekData.reduce((s, b) => s + (parseFloat(b.finalAmount) || parseFloat(b.amount) || 0), 0);
                const totalPending = weekData.reduce((b, bk) => {
                  const fin = parseFloat(bk.finalAmount) || parseFloat(bk.amount) || 0;
                  const paid = (parseFloat(bk.payGpay)||0)+(parseFloat(bk.payCash)||0)+(parseFloat(bk.payAdvance)||0);
                ...
                pdf.text(`Final Amount   : ₹${Math.round(totalFin).toLocaleString("en-IN")}`, 14, y + 14);
                pdf.text(`📱 GPay        : ₹${Math.round(totalGpay).toLocaleString("en-IN")}`, 14, y + 21);
                pdf.text(`💵 Cash        : ₹${Math.round(totalCash).toLocaleString("en-IN")}`, 14, y + 28);
                pdf.text(`🗓 Advance     : ₹${Math.round(totalAdv).toLocaleString("en-IN")}`, 14, y + 35);
                if (totalPending > 0) {
                  pdf.setTextColor(192, 57, 43);
                  pdf.text(`⏳ Pending Due  : ₹${Math.round(totalPending).toLocaleString("en-IN")}`, 14, y + 42);
                }
          ...
          {/* List */}
          {histLoading ? (
            <div style={{ textAlign: "center", padding: 28, color: "#aaa", fontSize: 13 }}>⏳ Loading…</div>
            ...
            filteredHistory.map((b) => (
              <HistoryCard key={b.id} booking={b} onClick={() => setDrawer(b)} />
            ))
          )}
        </div>
      )}
      ...
      {page === "settings" && (
        <div style={{ display: "flex", flexDirection: "column", flex: 1, paddingBottom: 80, overflowY: "auto" }}>
          <div style={{ background: "#1a472a", color: "#fff", padding: "14px 16px", display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <div><h1 style={{ fontSize: 18, fontWeight: 700, margin: 0 }}>Settings</h1><p style={{ fontSize: 11, opacity: 0.65, marginTop: 1 }}>Turf configuration</p></div>
          </div>
          <div style={{ padding: "12px 14px", flex: 1 }}>
            <div style={{ background: "#fff", borderRadius: 13, overflow: "hidden", marginBottom: 10 }}>
              {[
                { icon: "⚽", bg: "#e8f5e9", label: "Turf Name", key: "turfName", type: "text", placeholder: "My Turf Arena" },
                { icon: "💰", bg: "#fff3e0", label: "Weekday Rate (Mon–Fri) ₹", key: "wdPrice", type: "number", placeholder: "800" },
                { icon: "💰", bg: "#fce4ec", label: "Weekend Rate (Sat–Sun) ₹", key: "wePrice", type: "number", placeholder: "1000" },
              ].map(({ icon, bg, label, key, type, placeholder }) => (
                <div key={key} style={{ padding: "13px 14px", borderBottom: "1px solid #f5f5f5", display: "flex", alignItems: "flex-start", gap: 10 }}>
                  <div style={{ width: 34, height: 34, borderRadius: 8, background: bg, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 16, flexShrink: 0, marginTop: 2 }}>{icon}</div>
                  <div style={{ flex: 1 }}>
                    <h4 style={{ fontSize: 13, fontWeight: 700, marginBottom: 3 }}>{label}</h4>
                    <input type={type} value={settings[key] || ""} placeholder={placeholder}
                      onChange={(e) => setSettings((prev) => ({ ...prev, [key]: e.target.value }))}
                      style={{ width: "100%", border: "1.5px solid #e0e0e0", borderRadius: 9, padding: "9px 11px", fontSize: 13, marginTop: 7, outline: "none", background: "#fafafa", color: "#1a1a1a" }} />
                  </div>
                </div>
              ))}
              <div style={{ padding: "13px 14px", borderBottom: "1px solid #f5f5f5", display: "flex", alignItems: "flex-start", gap: 10 }}>
                <div style={{ width: 34, height: 34, borderRadius: 8, background: "#e8f5e9", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 16, flexShrink: 0, marginTop: 2 }}>🔗</div>
                <div style={{ flex: 1 }}>
                  <h4 style={{ fontSize: 13, fontWeight: 700, marginBottom: 3 }}>Apps Script URL</h4>
                  <p style={{ fontSize: 11, color: "#999" }}>Paste your Google Apps Script deployment URL</p>
                  <input type="text" value={settings.scriptUrl || ""} placeholder="https://script.google.com/macros/s/.../exec"
                    onChange={(e) => setSettings((prev) => ({ ...prev, scriptUrl: e.target.value }))}
                    style={{ width: "100%", border: "1.5px solid #e0e0e0", borderRadius: 9, padding: "9px 11px", fontSize: 11, marginTop: 7, outline: "none", background: "#fafafa", color: "#1a1a1a" }} />
                </div>
              </div>
              <div style={{ padding: "13px 14px" }}>
                <h4 style={{ fontSize: 13, fontWeight: 700, marginBottom: 7 }}>Slot Duration</h4>
                <select value={settings.slotDur || "60"} onChange={(e) => setSettings((prev) => ({ ...prev, slotDur: e.target.value }))}
                  style={{ width: "100%", border: "1.5px solid #e0e0e0", borderRadius: 9, padding: "9px 11px", fontSize: 13, outline: "none", background: "#fafafa" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              <button onClick={testConn} style={{ background: "#fff", color: "#1a472a", border: "1.5px solid #1a472a", borderRadius: 10, padding: 12, fontSize: 14, fontWeight: 700, cursor: "pointer" }}>
                🔌 Test Google Sheets Connection
              </button>
              <button onClick={syncAll} style={{ background: "#1a472a", color: "#fff", border: "none", borderRadius: 10, padding: 12, fontSize: 14, fontWeight: 700, cursor: "pointer" }}>
      ...
      {drawer && (
        <BookingDrawer
          ...
          moreGpay={moreGpay} setMoreGpay={setMoreGpay}
          moreCash={moreCash} setMoreCash={setMoreCash}
      ...
      {/* ═══════════════════════════════════════
          BOTTOM NAVIGATION
      ═══════════════════════════════════════ */}
      <div style={{ position: "fixed", bottom: 0, left: "50%", transform: "translateX(-50%)", width: "100%", maxWidth: 480, background: "#fff", borderTop: "1px solid #e0e0e0", display: "flex", zIndex: 100 }}>
        {[
          ...
          <button key={id} onClick={() => handleNav(id)}
            style={{ flex: 1, padding: "10px 4px 8px", display: "flex", flexDirection: "column", alignItems: "center", gap: 3, border: "none", background: "none", cursor: "pointer",
              color: page === id ? "#1a472a" : "#999", fontSize: 10, fontWeight: page === id ? 700 : 400 }}>
...
function SlotsList({ slots, dayBookings, date, onSlotClick, onAvailClick }) {
  ...
  slots.forEach((sl) => {
    ...
    if (b) {
      ...
      const borderColors = { Mix: "#1a472a", GPay: "#1565c0", Cash: "#b45309", Advance: "#5b21b6", Pending: "#e0e0e0" };
      ...
      rendered.push(
        <div key={b.id} onClick={() => onSlotClick(date, b)}
          style={{ background: "#fff", borderRadius: 12, marginBottom: 7, overflow: "hidden", boxShadow: "0 1px 2px rgba(0,0,0,0.05)", cursor: "pointer",
            borderLeft: `4px solid ${borderColors[mode] || "#e0e0e0"}` }}>
          <div style={{ padding: "10px 12px", display: "flex", alignItems: "center", gap: 9 }}>
            <div style={{ minWidth: 72 }}>
              <div style={{ fontSize: 11, fontWeight: 700, color: "#555" }}>{sl.label.split("–")[0].trim()}</div>
              <div style={{ fontSize: 9, color: "#bbb", marginTop: 1 }}>{sl.label.split("–")[1]?.trim()}</div>
            </div>
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ fontSize: 13, fontWeight: 700, color: "#1a1a1a", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{b.customerName}</div>
              <div style={{ fontSize: 10, color: "#999", marginTop: 2, display: "flex", alignItems: "center", gap: 5, flexWrap: "wrap" }}>
            ...
            <div style={{ textAlign: "right", flexShrink: 0 }}>
              <div style={{ fontSize: 14, fontWeight: 800, color: "#1a472a" }}>{fmtMoney(finalAmt)}</div>
              {pending > 0 && <div style={{ fontSize: 9, color: "#c0392b", fontWeight: 600 }}>Due: {fmtMoney(pending)}</div>}
            </div>
          </div>
        </div>
      );
      ...
      rendered.push(
        <div key={sl.label} onClick={() => onAvailClick(date, sl.label)}
          style={{ background: "#fff", borderRadius: 12, marginBottom: 7, overflow: "hidden", boxShadow: "0 1px 2px rgba(0,0,0,0.05)", cursor: "pointer", borderLeft: "4px solid #e8e8e8", opacity: 0.7 }}>
          <div style={{ padding: "10px 12px", display: "flex", alignItems: "center", gap: 9 }}>
            <div style={{ minWidth: 72 }}>
              <div style={{ fontSize: 11, fontWeight: 700, color: "#555" }}>{sl.label.split("–")[0].trim()}</div>
              <div style={{ fontSize: 9, color: "#bbb", marginTop: 1 }}>{sl.label.split("–")[1]?.trim()}</div>
            </div>
            <div style={{ fontSize: 12, color: "#ccc", flex: 1 }}>Available — tap to book</div>
...
function HistoryCard({ booking: b, onClick }) {
  const mode = getPayMode(b);
  const dotColors = { Mix: "#1a472a", GPay: "#1565c0", Cash: "#b45309", Advance: "#5b21b6", Pending: "#ccc" };
  const fin = parseFloat(b.finalAmount) || parseFloat(b.amount) || 0;
  const g = parseFloat(b.payGpay) || 0;
  const c = parseFloat(b.payCash) || 0;
  const a = parseFloat(b.payAdvance) || 0;
  ...
  return (
    <div onClick={onClick} style={{ background: "#fff", margin: "0 14px 7px", borderRadius: 12, padding: "12px 14px", display: "flex", alignItems: "center", gap: 10, boxShadow: "0 1px 2px rgba(0,0,0,0.05)", cursor: "pointer" }}>
      <div style={{ width: 9, height: 9, borderRadius: "50%", background: dotColors[mode] || "#ccc", flexShrink: 0 }} />
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ fontSize: 13, fontWeight: 700, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{b.customerName}</div>
        <div style={{ fontSize: 11, color: "#999", marginTop: 2 }}>{dStr} · {b.slotTime} · {b.sport}</div>
        <div style={{ marginTop: 4, display: "flex", gap: 4, flexWrap: "wrap" }}>
          {g > 0 && <span style={{ background: "#e3f2fd", color: "#1565c0", padding: "2px 7px", borderRadius: 5, fontSize: 10, fontWeight: 700 }}>📱 {fmtMoney(g)}</span>}
          {c > 0 && <span style={{ background: "#fffde7", color: "#b45309", padding: "2px 7px", borderRadius: 5, fontSize: 10, fontWeight: 700 }}>💵 {fmtMoney(c)}</span>}
          {a > 0 && <span style={{ background: "#ede7f6", color: "#5b21b6", padding: "2px 7px", borderRadius: 5, fontSize: 10, fontWeight: 700 }}>🗓 {fmtMoney(a)}</span>}
        </div>
      </div>
      <div style={{ textAlign: "right", flexShrink: 0 }}>
        <div style={{ fontSize: 15, fontWeight: 800, color: "#1a472a" }}>{fmtMoney(fin)}</div>
        {pend > 0 && <div style={{ fontSize: 10, color: "#c0392b", fontWeight: 600 }}>Due {fmtMoney(pend)}</div>}
      </div>
    </div>
  );
}
...
function WeeklySummary({ bookings: bList }) {
  ...
  const sunStr = sunday.toLocaleDateString("en-IN", { day: "numeric", month: "short" });

  return (
    <div style={{ margin: "4px 14px 8px", background: "#fff", borderRadius: 13, padding: "13px 14px", boxShadow: "0 1px 3px rgba(0,0,0,0.06)" }}>
      <div style={{ fontSize: 11, fontWeight: 700, color: "#888", textTransform: "uppercase", letterSpacing: 0.4, marginBottom: 10 }}>
        📅 This Week's Collections
      </div>
      <div style={{ fontSize: 10, color: "#bbb", marginBottom: 8 }}>{monStr} – {sunStr}</div>
      <div style={{ display: "flex", gap: 8 }}>
        {[
          ...
          <div key={label} style={{ flex: 1, background: bg, borderRadius: 10, padding: 10, textAlign: "center" }}>
            <div style={{ fontSize: 10, fontWeight: 700, color }}>{label}</div>
            <div style={{ fontSize: 18, fontWeight: 800, color, marginTop: 3 }}>{fmtMoney(val)}</div>
          </div>
        ))}
      </div>
    </div>
...
function BookingDrawer({ booking: b, onClose, onDelete, onCollect, collectMode, moreGpay, setMoreGpay, moreCash, setMoreCash, onSubmitMore }) {
  const finalAmt = parseFloat(b.finalAmount) || parseFloat(b.amount) || 0;
  const g = parseFloat(b.payGpay) || 0;
  const c = parseFloat(b.payCash) || 0;
  const a = parseFloat(b.payAdvance) || 0;
    ...
    <div
      ...
      <div style={{ background: "#fff", borderRadius: "20px 20px 0 0", width: "100%", maxWidth: 480, maxHeight: "94vh", overflowY: "auto" }}>
        ...
        <div style={{ padding: "14px 18px", display: "flex", alignItems: "center", justifyContent: "space-between", borderBottom: "1px solid #f0f0f0", position: "sticky", top: 0, background: "#fff" }}>
          <h3 style={{ fontSize: 16, fontWeight: 700, margin: 0 }}>{collectMode ? "Collect Payment" : b.slotTime}</h3>
        ...
        <div style={{ padding: "14px 18px" }}>
          {collectMode ? (
            <>
              ...
              <div style={{ background: "#f8f8f8", borderRadius: 10, padding: 12, marginBottom: 13 }}>
                {[{ label: "📱 GPay", val: moreGpay, set: setMoreGpay }, { label: "💵 Cash", val: moreCash, set: setMoreCash }].map(({ label, val, set }) => (
                  <div key={label} style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
                    <div style={{ fontSize: 12, color: "#777", fontWeight: 600, minWidth: 58 }}>{label}</div>
                    <input type="number" value={val || ""} onChange={(e) => set(parseFloat(e.target.value) || 0)} placeholder="0"
                      style={{ flex: 1, border: "1.5px solid #e0e0e0", borderRadius: 8, padding: "8px 10px", fontSize: 14, background: "#fff", outline: "none" }} />
              ...
              <button onClick={() => onSubmitMore(b)} style={{ width: "100%", padding: 14, background: "#1a472a", color: "#fff", border: "none", borderRadius: 12, fontSize: 16, fontWeight: 700, cursor: "pointer" }}>
                ✅ Add Payment
              </button>
            </>
          ) : (
            <>
              <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 14 }}>
                <div style={{ width: 46, height: 46, borderRadius: 12, background: "#e8f5e9", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 20 }}>👤</div>
                <div>
              ...
              <div style={{ background: "#f8f8f8", borderRadius: 12, padding: 14, marginBottom: 14 }}>
                {[
                  ["Date", new Date(b.date + "T00:00:00").toLocaleDateString("en-IN", { weekday: "short", day: "numeric", month: "short", year: "numeric" }), ""],
                  ["Slot", b.slotTime, ""],
                  ["Original Amount", fmtMoney(b.amount), ""],
                  ...(b.discount > 0 ? [["Discount", `-${fmtMoney(b.discount)}`, "red"]] : []),
                  ["Final Amount", fmtMoney(finalAmt), "green"],
                  ["Total Paid", fmtMoney(paid), "blue"],
                  ...(pending > 0 ? [["Balance Due", fmtMoney(pending), "red"]] : []),
                  ...
                  <div key={lbl} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 9 }}>
                    ...
                    <div style={{ fontSize: 13, fontWeight: 700, color: color === "green" ? "#1a6b38" : color === "red" ? "#c0392b" : color === "blue" ? "#1565c0" : "#1a1a1a", textAlign: "right" }}>{val}</div>
                  </div>
                ))}
              </div>
              <button onClick={() => onCollect(b)} style={{ width: "100%", padding: 14, background: "#1a472a", color: "#fff", border: "none", borderRadius: 12, fontSize: 16, fontWeight: 700, cursor: "pointer", marginBottom: 8 }}>
